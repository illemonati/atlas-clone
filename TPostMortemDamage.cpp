...
//TPMDTables
//---------------------------------------------------------------
TPMDTables::TPMDTables(TReadGroups* ReadGroups, int maxLengthForInference, int MaxReadLength, TReadGroupMap &ReadGroupMap):readGroupMapObject(ReadGroupMap){
...
void TPMDTables::fitExponentialModel(int numNRIterations, double eps, std::string & filename, TLog* logfile){
	...
	for(int i=0; i<readGroups->numGroups; ++i){
		if(readGroups->inUse[i]) out << readGroups->getName(i) << "\t" << forward[readGroupMapObject[i]]->fitExponentialModel(C, T, numNRIterations, eps, readGroups->getName(i), maxReadLength, logfile)
