...
//TFastaBuffer
//-----------------------------------------------------
TFastaBuffer::TFastaBuffer(Fasta* Reference){
    bufferSize = 100000;
    reference = Reference;
    referenceSequence = "";
    curStart = -1;
...
void TAlignmentParser::init(int MaxReadLength, TParameters & params, TLog* Logfile){
    ...
    bamReader = IOTool::getInstance()->createBamReader();
...
void TAlignmentParser::openBamFile(std::string filename){
    ...
    bamReader->Jump(bamHeader->GetSequences().Size() - 1, 0);
    BamAlignment* bamAlignment=IOTool::getInstance()->createBamAlignment();
...
void TAlignmentParser::setMasks(TParameters & params){
    ...
    if(params.parameterExists("mask")){
        ...
        mask = new TBedReader(maskFile, windowSize, bamHeader->GetSequences(), logfile);
        logfile->done();
        logfile->endIndent();
        //mask->print();
    ...
    if(params.parameterExists("regions")){
        ...
        logfile->listFlush("Reading file ...");
        mask = new TBedReader(regionsFile, windowSize, bamHeader->GetSequences(), logfile);
        logfile->done();
        logfile->endIndent();
...
void TAlignmentParser::addReference(Fasta* reference){
    hasReference = true;
...
std::string TAlignmentParser::chrNumberToName(int chrNumber){
    int counter = 0;
    SamSequenceIter* iter = bamHeader->GetSequences().CreateIterator();
    while(iter->HasNext()){
        SamSequence& sequence = iter->Next();
        if(chrNumber==counter){
...
bool TAlignmentParser::readAlignment(){
    ...
    do {
        ...
        if(bamAlignment->GetRefID() != previousAlignmentChr){
            previousAlignmentPos = -1;
            previousAlignmentChr = bamAlignment->GetRefID();
//			chrNumber = previousAlignmentChr;
            chrChangedAlignment = true;
        } else
            chrChangedAlignment = false;

        if(bamAlignment->GetPosition() < previousAlignmentPos)
            throw "BAM file must be sorted by position! Alignment '" + bamAlignment->GetName() + "' is at position " + toString(bamAlignment->GetPosition()) + ", which is before the position of the previous alignment (" + toString(previousAlignmentPos) + ")";
        previousAlignmentPos = bamAlignment->GetPosition();
...
//		}
        ...
        if(bamAlignment->GetAlignedBases().size() > maxReadLength)
            throw "Alignment '" +  bamAlignment->GetName() + "' is longer than the max read length! Please change max read length to parse this data.";

        //store read group ID
        std::string readGroup;
        std::string tag="RG";
        bamAlignment->GetZTag(tag, readGroup);
...
bool TAlignmentParser::applyFilters(){
    ...
    bool filtersPassed = readGroups.readGroupInUse(curReadGroupID)
                    ...
                    && chromosomes.inUse(bamAlignment->GetRefID())
...
bool TAlignmentParser::readNextAlignmentWithBlacklist(TAlignment & alignment){
    ...
    if(readAlignment()){
        ...
        blacklist.emplace(bamAlignment->GetName(), 1);
    }
    return false;
};
...
void TAlignmentParser::readAlignmentsIntoWindow(TWindow & window){
    ...
    if(oldAlignmentMustBeConsidered){
        if(bamAlignment->GetPosition() >= window.end){
            return;
        }
