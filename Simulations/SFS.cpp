...
void SFS::_fillFrequencies(){
	...
	for(uint32_t i=1; i<sfs.size(); ++i){
		sfsFrequencies.push_back((float) i / (float) numChromosomes());
	}
};
...
void SFS::_fillCumulative(){
	...
	for(uint32_t i=1; i<sfs.size(); ++i){
		sfsCumulative.push_back(sfsCumulative[i-1] + sfs[i]);
	}
...
SFSfolded::SFSfolded(uint32_t numChr, const float & theta) {
	...
	float sum = 0;
	for(uint32_t i=1; i<numChr + 1; ++i){
		sfs.push_back(theta/i + theta/(numChromosomes() - i));
		sum += sfs.back();
	}
	if(sum > 1.0)
		throw "The choice of theta and sample size results in too many mutations in the SFS!";
...
double SFSfolded::calcLLOneSite(float* gl){
	...
	int j = dimension-1;
	for(uint32_t i=dimension; i<2 * dimension - 1; ++i, --j)
		LL += exp(gl[i]) * sfs[j];
