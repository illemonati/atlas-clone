...
TGenome::TGenome(const BAM::TBamFilters &Filters) : TGenome(parameters().get<std::string>("bam"), Filters, -1) {}

TGenome::TGenome(std::string_view Name, const BAM::TBamFilters &Filters, size_t i)
    : _bamFile(Name), _rgInfo(_bamFile.readGroups()), _errorModels(_rgInfo) {
	if (parameters().exists("out")) {
		_outputName = parameters().get("out");
		if (i != size_t(-1)) { _outputName += coretools::str::toString("_", i); }
