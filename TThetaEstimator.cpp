...
TThetaEstimator::TThetaEstimator(TParameters & params, TLog* Logfile):TThetaEstimator_base(params, Logfile){
    ...
    maxEpsilon = params.getParameterDoubleWithDefault("maxEps", 1e-15);
...
void TThetaEstimator::fillP_G(){
    ...
    memset(P_G, 0, sizeof(double)*numGenotypes);
...
void TThetaEstimator::runEMForTheta(){
	...
	//start EM loop
    int iter = 0;
    double oldTheta = data->sizeWithData();
    do{
        ...
        if(abs(theta.LL - oldLL) < maxEpsilon)
			break;
        ...
        iter++;
    }while(abs(oldTheta - theta.theta)>maxEpsilon && iter < numIterations);
