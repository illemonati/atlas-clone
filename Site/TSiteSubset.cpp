...
TSiteSubsetChr::TSiteSubsetChr(std::string &Name, int &WindowSize, SamHeader &bamHeader){
    name = Name;
    windowSize = WindowSize;
    ...
    for(;iterator->HasNext(); ++i){
        ...
        if(sequence.GetName() == name) chrNumberInFasta = i;
    }
...
bool TSiteSubsetChr::addPosition(std::vector<std::string> &tmp, const std::string &chr, Fasta &reference, std::string &error, bool invariantSites){
    long pos = stringToLong(tmp[1]) - 1; //make 0-based
    char ref = tmp[2][0];
    char alt = tmp[3][0];
...
void TSiteSubset::readFile(Fasta &reference, SamHeader &bamHeader, TLog *logfile){ //version that checks witth fasta reference
    logfile->listFlush("Reading sites to be used from '" + filename + "' ...");
    //open file
    std::ifstream sitesFile(filename.c_str());
    if(!sitesFile) throw "Failed to open sites file '" + filename + "'!";
...
TSiteSubset::TSiteSubset(std::string Filename, Fasta &reference, SamHeader &bamHeader, int &WindowSize, TLog *logfile, bool InvariantSites){
    filename = Filename;
    windowSize = WindowSize;
    invariantSites = InvariantSites;
