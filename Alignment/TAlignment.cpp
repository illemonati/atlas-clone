...
void TAlignment::fill(BamAlignment & BamAlignment, int ReadGroupId){
	//clear
	clear();

	//add basic info
    bamAlignment = &BamAlignment;
    chrNumber = bamAlignment->GetRefID();
    position = bamAlignment->GetPosition();
    alignmentName = bamAlignment->GetName();
    isReverseStrand = bamAlignment->IsReverseStrand();
    isPaired = bamAlignment->IsPaired();
    isProperPair = bamAlignment->IsProperPair();
    mappingQuality = bamAlignment->GetMapQuality();
	readGroupId = ReadGroupId;
    insertSize = bamAlignment->GetInsertSize();
    isSecondMate = bamAlignment->IsSecondMate();
    matePosition = bamAlignment->GetMatePosition();
...
void TAlignment::parseBasesQualities(TGenotypeMap & genoMap, TQualityMap & qualityMap){
    ...
    for( ; cigarIter.HasNext(); ++counter ){
        ...
        switch ( op.GetType() ) {
            ...
            case ('X') :
                ...
                for(unsigned int i=0; i<op.GetLength(); ++i, ++d, ++p){
                    bases[d].base = genoMap.getBase(bamAlignment->GetQueryBases()[d]);
                    qualityOriginal[d] = (int) bamAlignment->GetQualities()[d];
                    bases[d].errorRate = qualityMap.qualityToErrorMap[(int) bamAlignment->GetQualities()[d]];
            ...
            case ('S') :
                ...
                for(unsigned int i=0; i<op.GetLength(); ++i, ++d){
					//soft-clipped bases on 5' are before bamAlignment.Position
                    softClippedBase[softClippedEntry][softClippedLength[softClippedEntry]] = bamAlignment->GetQueryBases()[d];
                    softClippedQuality[softClippedEntry][softClippedLength[softClippedEntry]] = bamAlignment->GetQualities()[d];
            ...
            case ('I')      :
                for(unsigned int i=0; i<op.GetLength(); ++i, ++d){
                    bases[d].base = genoMap.getBase(bamAlignment->GetQueryBases()[d]);
                    qualityOriginal[d] = (int) (char) bamAlignment->GetQualities()[d];
                    bases[d].errorRate = qualityMap.qualityToErrorMap[(int) bamAlignment->GetQualities()[d]];
					bases[d].aligned = false;
					++numInsertions;
					bases[d].alignedPos = -1;
    ...
    if(passedFilters && length != bamAlignment->GetLength())
        throw "The lengths of the alignment and the quality scores of read '" + bamAlignment->GetName() + "' do not match!";
};
...
void TAlignment::updateOptionalSamField(std::string tag, float value){
    if(bamAlignment->HasTag(tag) == false) bamAlignment->AddFloatTag(tag, value);
    else bamAlignment->EditFloatTag(tag, value);
};

void TAlignment::updateOptionalSamField(std::string tag, std::string value){
    if(bamAlignment->HasTag(tag) == false) bamAlignment->AddZTag(tag, value);
    else bamAlignment->EditZTag(tag, value);
};
...
void TAlignment::assessSoftClipping(int & S_left, int & middle, int & S_right, std::string & S_string_left, std::string & S_string_middle, std::string & S_qualities_middle, std::string & S_string_right, TGenotypeMap & genoMap){
    ...
    while(cigarIter.HasNext()){
        ...
        if(op.GetType() == 'S'){
			if(reachedMiddle){
                S_right += op.GetLength();
                for(unsigned int i=0; i<op.GetLength(); ++i, ++p)
                    S_string_right += bamAlignment->GetQueryBases()[p];
			}
			else{
                S_left += op.GetLength();
                for(unsigned int i=0; i<op.GetLength(); ++i, ++p)
                    S_string_left += bamAlignment->GetQueryBases()[p];
			}
		} else {
            if(op.GetType() == 'D')
				continue;
            ...
            middle += op.GetLength();
            for(unsigned int i=0; i<op.GetLength(); ++i, ++p){
                S_string_middle += bamAlignment->GetQueryBases()[p];
                S_qualities_middle += bamAlignment->GetQueryBases()[p];
			}
		}
	}
...
void TAlignment::setToSingleEnd(){
    bamAlignment->SetLength(bamAlignment->GetAlignedBases().size());
    bamAlignment->GetCigarData().clear();
    CigarOp* cigar = IOTool::getInstance()->createCigarOp('M',bamAlignment->GetLength());
    bamAlignment->GetCigarData().push_back(*cigar);
    ...
    bamAlignment->SetMateRefID(-1);
    bamAlignment->SetMatePosition(-1);
    bamAlignment->SetInsertSize(0);
}
...
void TAlignment::save(BamWriter & bamWriter, TGenotypeMap & genoMap, int & minQualForPrinting, int & maxQualForPrinting, TQualityMap & qualMap){
	if(changed){
...
//		tmpString2.clear();
        ...
        bamAlignment->SetQueryBases(tmpString);
        bamAlignment->SetQualities(tmpString2);
	}

	//make sure quality are printed within range

    std::string qualities = bamAlignment->GetQualities();
    filterForPrintingBaseQuality(qualities, minQualForPrinting, maxQualForPrinting);

	//now write alignment
    if(!bamWriter.SaveAlignment(*bamAlignment))
        throw "Read '" + bamAlignment->GetName() + "' could not be written!";
};

void TAlignment::print(TGenotypeMap & genoMap, TQualityMap & qualMap){
	std::cout << "NAME:\t" << alignmentName << std::endl;
    std::cout << "aligned bases " << bamAlignment->GetAlignedBases() << std::endl;
