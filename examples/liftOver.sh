...
$atlas liftOver --mode Bed2Fastq --bed ${in}.bed --fasta simulate.fasta --flank $flank\
	   --fixedSeed 42 --out $out --logFile $out.out 2> $out.eout
	   
# compare read names in fastq (chrLength = 1000, see above)
awk -vflank=$flank -vdelim=$delim '{for(i=$2; i<$3; ++i){ start=i-flank; if(start < 0){start=0}; end=i+flank+1; if(end>1000){end = 1000}; print "@" $1 ":" start "-" end delim i-start delim $4}}' ${in}.bed > expectedNames.txt
awk '$1~/@/' ${out}.fastq > atlasNames.txt
diff expectedNames.txt atlasNames.txt >> $out.eout

# compare sequences
awk -vflank=$flank '{for(i=$2; i<$3; ++i){ start=i-flank+1; if(start < 1){start=1}; end=i+flank+1; if(end>1000){end = 1000}; print $1 ":" start "-" end}}' ${in}.bed | while read line
do
...
( 
	...
	echo "ch1:425-436${delim}5${delim}mapping6M1I4M 0 chr1 426 50 6M1I4M * 0 11 AAAAAAAAAAA 12345678901" # mapped with insertion
	...
	echo "ch2:946-957${delim}5${delim}conflictingPos 0 chr2 947 50 5M1D6M * 0 11 AAAAAAAAAAA 12345678901" # double entry at same position: both should be ignored
	echo "ch2:946-957${delim}5${delim}conflictingPos 0 chr2 947 50 5M1D6M * 0 11 AAAAAAAAAAA 12345678901" # double entry at same position: both should be ignored
	echo "ch3:15-26${delim}5${delim}conflictingInfo1 0 chr3 16 50 11M * 0 11 AAAAAAAAAAA 12345678901" # double entry at same position with conflicting info: both should be ignored
	echo "ch3:15-26${delim}5${delim}conflictingInfo2 0 chr3 16 50 11M * 0 11 AAAAAAAAAAA 12345678901" # double entry at same position with conflicting info: both should be ignored
...
for i in {0..9}; do
	...
	echo "ch3:${start}-${end}${delim}5${delim}Info2 0 chr3 $pos 50 11M * 0 11 AAAAAAAAAAA 12345678901" | tr ' ' '\t' >> testBam.sam
done
...
awk 'BEGIN {OFS="\t"} {if ($2+1==$3) {print $1, $2, $3, $4} else {pos0=$2; while (pos0 < $3) {print $1, pos0, pos0+1, $4; pos0++}}}' ${in}.bed > expectedBed.bed
