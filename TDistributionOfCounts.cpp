...
TDistributionOfCounts::TDistributionOfCounts(int MaxValue, std::string nameOfCounts){
...
TDistributionOfCounts::~TDistributionOfCounts(){
    delete[] counts;
}



void TDistributionOfCounts::add(int depth){
    if(depth > maxValue)
        ++counts[lastBin];
    else
        ++counts[depth];
...
void TDistributionOfCounts::add(int depth, int number){
    if(depth > maxValue)
...
unsigned int TDistributionOfCounts::totNumSites(){
    unsigned int sum = 0;
    for(int i=0; i<numBins; ++i)
        sum += counts[i];
    return sum;
...
void TDistributionOfCounts::writeCounts(std::string filename){
    //open file
...
void TDistributionOfCounts::writeNormalizedCounts(std::string filename){
    //open file
    TOutputFilePlain out(filename);
...
void TDistributionOfCounts::writeCumulativeCounts(std::string filename){
    //open file
    TOutputFilePlain out(filename);
...
void TDistributionOfCounts::writeNormalizedCumulativeCounts(std::string filename){
    //open file
    TOutputFilePlain out(filename);
...
void TDistributionOfCounts::writeQuantiles(std::string filename){
    //open file
    TOutputFilePlain out(filename);
