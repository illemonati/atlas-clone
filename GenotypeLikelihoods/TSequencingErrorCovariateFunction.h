...
public:
	...
	//size
	uint16_t firstParameterIndex() const noexcept {return _firstParameterIndex;};
...
public:
	...
	void fillDerivatives(TRecalibrationEMFirstDerivatives & first, TRecalibrationEMSecondDerivatives & second) const noexcept;
	void fillDerivatives(uint16_t val, TRecalibrationEMFirstDerivatives & first, TRecalibrationEMSecondDerivatives & second) const noexcept override;
...
private:
	...
	double _getAsDouble(uint16_t val) const noexcept {return _transformationMap ? _transformationMap->get(val) : val;};
protected:
...
public:
	...
	void fillDerivatives(uint16_t val, TRecalibrationEMFirstDerivatives & first, TRecalibrationEMSecondDerivatives & second) const noexcept override;
	double adjustParametersPostEstimation() noexcept override { return _normalizeParameters(); };
	virtual std::string typeString() const noexcept override {return name; }
};
