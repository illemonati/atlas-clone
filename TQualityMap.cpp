...
double TQualityMap::phredIntToError(int phredInt){
    if(phredInt >= minPhredInt)
        return min;
    else return phredIntToErrorMap[phredInt];
}

double TQualityMap::qualityToError(int qual){
    if(qual < 33)
        return 1.0;
    return phredIntToError(qual - 33);
}

int TQualityMap::phredIntToQuality(int phredInt){
    return phredInt + 33;
}

double &TQualityMap::operator[](int phred){
    //Note: no check on range!
    return phredIntToErrorMap[phred];
}

double &TQualityMap::operator[](uint8_t &phred){
    //Note: no check on range!
    return phredIntToErrorMap[phred];
}
