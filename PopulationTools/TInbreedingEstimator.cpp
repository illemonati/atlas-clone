...
TInbreedingEstimatorPrior::TInbreedingEstimatorPrior(
    stattools::TParameterTyped<TypeF, 1> *F,
...
void TInbreedingEstimatorPrior::_readCommandLineArguments() {
  ...
  _q_FModel_To_HWE =
      parameters().getParameterWithDefault("probMovingToModelNoF", 0.1);
  _log_q_FModel_To_HWE = log(_q_FModel_To_HWE);
  logfile().list("Will propose move to model without F with probability ",
                 _q_FModel_To_HWE, ". (parameter 'probMovingToModelNoF')");
  ...
  logfile().list("Lambda of exponential distribution used for the proposal of "
                 ...
                 coretools::str::toString(_lambdaNewF),
                 ". (parameter 'lambdaF')");
}

coretools::LogProbability
TInbreedingEstimatorPrior::_calculateLLSumOverIndividuals(
    const Storage &Data, size_t Locus,
...
coretools::LogProbability
TInbreedingEstimatorPrior::_calculateLLSumOverIndividuals(const Storage &Data,
                                                          size_t Locus,
                                                          double P) const {
  return _calculateLLSumOverIndividuals(
      Data, Locus, genometools::THardyWeinbergGenotypeProbabilities(P));
}
...
double TInbreedingEstimatorPrior::_calculateProbabilityOfProposingThisF(
    double F) const {
...
void TInbreedingEstimatorPrior::_updateRegularF(const Storage &Data) {
  ...
  if (_F->update()) {
    ...
    for (size_t l = 0; l < _numLoci; l++) {
      logH +=
          _calculateLLSumOverIndividuals(Data, l, _p->value(l), _F->value()) -
...
void TInbreedingEstimatorPrior::_updateHWEToF(const Storage &Data) {
  ...
  // calculate RJ-MCMC term of Hastings ratio
  double log_f_F = _calculateProbabilityOfProposingThisF(
      _F->value()); // "compensating" the effect of no F in HWE model
  ...
  if (_F->acceptOrReject_DontCountRejection(logH)) {
    ...
    _F->doTrackMeanVar(
        true); // F is in non-zero model -> add to posterior mean/var
  } else {
...
double
TInbreedingEstimatorPrior::getSumLogPriorDensity(const Storage &Data) const {
  double sum = 0.;
  for (size_t l = 0; l < _numLoci; l++) {
    if (_z->value()) {
...
void TInbreedingEstimatorPrior::simulateUnderPrior() {
  throw std::runtime_error((std::string) __PRETTY_FUNCTION__ +
                           ": Not implemented. Use Atlas simulator instead.");
}
...
TInbreedingEstimatorModel::TInbreedingEstimatorModel(const std::string &Filename,
                                                     std::shared_ptr<stattools::TDAGBuilder> & DAGBuilder,
    ...
    : _F("F", std::make_shared<stattools::prior::TUniformFixed<stattools::TParameterBase, TypeF, 1>>(), {Filename}),
      ...
      _log_alpha("log_alpha", std::make_shared<stattools::prior::TUniformFixed<stattools::TParameterBase, TypeLogAlpha, 1>>(), {Filename}),
      _log_beta("log_beta", std::make_shared<stattools::prior::TUniformFixed<stattools::TParameterBase, TypeLogAlpha, 1>>(),{Filename}),
      _p("p", std::make_shared<stattools::prior::TBetaInferred<stattools::TParameterBase, TypeP, 1, TypeLogAlpha, TypeLogBeta, true, true>>(&_log_alpha, &_log_beta), {Filename}),
      _observation("genotypeLikelihoods", std::make_shared<TInbreedingEstimatorPrior>(&_F, &_p, &_z, Likelihoods.alleleFrequencies()),{}) {
...
void TInbreedingEstimator::_readData() {
  ...
  if (parameters().parameterExists("trueAlleleFreq")) {
    ...
    logfile().list(
        "Will save true allele frequencies for population likelihoods.");
  }
